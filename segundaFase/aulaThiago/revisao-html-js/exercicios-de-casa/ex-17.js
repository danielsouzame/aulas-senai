...
function usuariosMenores(array){
  const menores = array.filter(usuario => usuario.idade < 18 && usuario.temAnimalEstimacao === true);

  menores.forEach(usuario => {
    console.log(`${usuario.nome} ${usuario.sobrenome}, ${usuario.idade} anos e tem Animal de estimação.`)
  });
}

usuariosMenores(usuarios)